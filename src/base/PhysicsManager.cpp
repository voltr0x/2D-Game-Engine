...
bool PhysicsManager::getCollisions(float rx, float ry, float rw, float rh, std::vector<std::shared_ptr<GameObject>>& objects) const {
  b2AABB box;
  box.lowerBound.Set(rx * PhysicsManager::GAME_TO_PHYSICS_SCALE, ry * PhysicsManager::GAME_TO_PHYSICS_SCALE);
  box.upperBound.Set((rx + rw) * PhysicsManager::GAME_TO_PHYSICS_SCALE, (ry + rh) * PhysicsManager::GAME_TO_PHYSICS_SCALE);

  objects.clear();
  PhysicsCallback callback(objects);
  mWorld->QueryAABB(&callback, box);
  return !objects.empty();  // TODO PART4: use your b2QueryCallback subclass to get collisions
}
